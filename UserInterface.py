...
class UserInterface(RestApi):
  ...
  def get_raw_user_choice(self):
    try:
      user_raw_choice = int(input('Hello! What do you want to do?\n'))
    except ValueError:
      print('I don\'t understand!')
      self.quit_from_shop()
    return user_raw_choice

  def run_user_choice(self, user_raw_choice):
    ...
    switcher.get(user_raw_choice, self.quit_from_shop)()

  def main(self):

    while True:
      self.print_exchange_rates()
      self.print_avaible_options()
      user_raw_choice = self.get_raw_user_choice()
      self.run_user_choice(user_raw_choice)
